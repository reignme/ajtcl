...
AJ_Status AJ_GetLocalGUID(AJ_GUID* localGuid)
{
    ...
    if (AJ_NVRAM_Exist(AJ_LOCAL_GUID_NV_ID)) {
        ...
        if (handle) {
            uint16_t size = AJ_NVRAM_Read(localGuid, sizeof(AJ_GUID), handle);
        ...
        if (handle) {
            uint16_t size = AJ_NVRAM_Write(localGuid, sizeof(AJ_GUID), handle);
            AJ_ASSERT(sizeof(AJ_GUID) == size);
            AJ_NVRAM_Close(handle);
            status = AJ_OK;
        }
...
AJ_Status AJ_GetRemoteCredential(const AJ_GUID* peerGuid, AJ_PeerCred* peerCreds)
{
    ...
    if (id > 0) {
        ...
        if (handle) {
            uint16_t size = AJ_NVRAM_Read(peerCreds, sizeof(AJ_PeerCred), handle);
